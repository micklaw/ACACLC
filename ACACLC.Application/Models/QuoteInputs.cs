...
namespace ACACLC.Application.Models
{
    public class QuoteInputs : AbstractValidator<QuoteInputs>
    {
        ...
        public QuoteInputs()
        {
            ...
            RuleFor(x => x.DeliveryDate).NotNull().WithMessage("Customer must provide a delivery date.").LessThan(DateTime.Now).WithMessage("Delivery date cannot be in the past.");
            RuleFor(x => x.NumberOfYears).NotNull().GreaterThan(0).LessThanOrEqualTo(3).WithMessage($"Payment term must be between {AvailableDurations.Min()}-{AvailableDurations.Max()} years");
            RuleFor(x => x.Deposit).NotNull().GreaterThan(0).Must(BeWithinMinimumDeposit).WithMessage($"Deposit £{Deposit} must be greater than 0 and within {MinimumDepositDisplay}% of the vehicle price £{VehiclePrice}.");
            ...
            RuleFor(x => x.VehiclePrice).NotNull().GreaterThanOrEqualTo(0).WithMessage($"Vehicle price £{VehiclePrice} cannot be a negative number.");
        }
